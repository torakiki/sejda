...
public class PdfAlternateMixer extends PDDocumentHandler {
    ...
    public void mix(NotifiableTaskMetadata taskMetadata) throws TaskException {
        ...
        List<PDPage> firstSourcePages = firstDocumentHandler.getUnderlyingPDDocument().getDocumentCatalog()
                .getAllPages();
        List<PDPage> secondSourcePages = secondDocumentHandler.getUnderlyingPDDocument().getDocumentCatalog()
                .getAllPages();

        PdfMixInputProcessStatus firstDocStatus = firstInput.newProcessingStatus(firstSourcePages.size());
        PdfMixInputProcessStatus secondDocStatus = secondInput.newProcessingStatus(secondSourcePages.size());

        int currentStep = 0;
        int totalSteps = firstSourcePages.size() + secondSourcePages.size();
