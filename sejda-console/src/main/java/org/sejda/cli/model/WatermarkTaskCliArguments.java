...
public interface WatermarkTaskCliArguments extends CliArgumentsWithPdfAndDirectoryOutput,
    ...
    @Option(shortName = "l", description = "watermark location relative to the page content. { behind, over }  (optional)", defaultValue = "behind")
    WatermarkLocationAdapter getLocation();

    @Option(shortName = "a", description = "watermark opacity, between 0 (transparent) and 100 (fully opaque). (optional)", defaultValue = "100")
    Integer getAlpha();
